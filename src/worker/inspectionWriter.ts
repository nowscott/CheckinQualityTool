...
const TRACE_COLUMNS = ["抽检序号", "抽检类型", "入选原因", "业务周", "抽取尝试次数", "源表行号", "课次ID", "教师邮箱", "报告链接H5", "系统是否生成报告", "报告生成时间", "产品分组", "项目组", "校区", "科目"] as const;
const RISK_TRACE_COLUMNS = ["是否抽检", "抽检序号", "抽检类型", "抽检原因", "风险判断", "源表行号", "课次ID", "教师邮箱", "报告链接H5", "系统是否生成报告", "报告生成时间", "产品分组", "项目组", "校区", "科目"] as const;
...
export function buildInspectionOutput(selection: InspectionSelection, includeExplanation: boolean): InspectionOutput {
  ...
  const sheets: SheetDefinition[] = [
    {
      ...
      widths: {
        ...
        抽检序号: 10, 抽检类型: 16, 入选原因: 32, 业务周: 24, 抽取尝试次数: 12, 源表行号: 10,
        课次ID: 20, 教师邮箱: 30, 报告链接H5: 34, 系统是否生成报告: 18, 报告生成时间: 22,
    ...
    },
    {
      ...
      widths: {
        ...
        是否具有个性化点评: 26, 是否具有图文并茂: 24, 是否具有行课计划: 24, 拟扣罚: 16,
        [TEMPLATE_COLUMNS[9]]: 36, [TEMPLATE_COLUMNS[10]]: 50, [TEMPLATE_COLUMNS[11]]: 50, [TEMPLATE_COLUMNS[12]]: 50,
        是否抽检: 12, 抽检序号: 10, 抽检类型: 16, 抽检原因: 34, 风险判断: 24, 源表行号: 10,
        课次ID: 20, 教师邮箱: 30, 报告链接H5: 34, 系统是否生成报告: 18, 报告生成时间: 22,
        产品分组: 30, 项目组: 28, 校区: 24, 科目: 12,
      },
      rowStyle: () => 32,
  ...
  if (includeExplanation) {
    const explanation: DataRow[] = [
      ...
      { 项目: "导出优先级", 值: selection.priority.mode === "unreported" ? "优先本月未反馈教师" : "优先覆盖不同教师" },
      ...
      { 项目: "管理岗位排除课程数", 值: selection.stats.excludedManagementRows },
      { 项目: "管理岗位排除教师数", 值: selection.stats.excludedManagementTeachers },
      ...
      { 项目: "排除原因", 值: `无邮箱 ${selection.stats.excludedNoEmailRows} 条；邮箱不在职 ${selection.stats.excludedNotInRosterRows} 条；岗位含“主管”或“经理”或命中岗位纠偏名单 ${selection.stats.excludedManagementRows} 条，均不参与抽检` },
