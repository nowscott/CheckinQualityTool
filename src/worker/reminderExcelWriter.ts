...
const TEACHER_COLUMNS = [
  "教师姓名", "教研组", "师训组长", "助理主管",
  "应发送人次", "已发送人次", "发送差额", "申诉数", "发送进度", "是否达标",
] as const;
...
function buildPublicTable(matchInfo: ReminderMatchInfo): PublicTable {
  ...
  const ownTeachingGroups = assistantOwnTeachingGroups(matchInfo.studentRows);
...
function explanationRows(
  ...
  const rows: DataRow[] = [
    ...
    { 项目: "申诉公示规则", 值: "上传申诉文件后，按教师姓名+学生姓名匹配分母；先判定聊天是否已发送，已发送优先展示为“是”并计入已发送，不计入申诉数；未发送但申诉原因或状态选择“已发送”的行按“是”计入应发送和已发送；其他申诉行标注“已申诉”、展示申诉原因，且不计入应发送、未发送和发送率分母；各汇总页单独公示申诉数" },
  ];
