...
const TEACHER_COLUMNS = [
  "教师姓名", "教研组", "师训组长", "助理主管",
  "应发送条数", "已发送数", "发送差额", "申诉数", "发送率", "是否达标",
] as const;

const TRAINING_COLUMNS = [
  "教研组", "应发送数", "已发送数", "申诉数", "发送率", "是否达标（80%）",
] as const;

const ASSISTANT_COLUMNS = [
  "教研组", "助理主管", "应发送数", "已发送数", "申诉数", "发送率", "是否达标（80%）",
] as const;

const PROJECT_COLUMNS = [
  "教研组", "负责人", "应发送数", "已发送数", "申诉数", "发送率", "是否达标（80%）",
] as const;
...
function createBucket(): SummaryBucket {
  return { total: 0, sent: 0, appealed: 0, counselorSent: 0, assistants: new Set() };
}
...
function addTeacher(bucket: SummaryBucket, row: DataRow) {
  ...
  bucket.sent += Number(row.已发送数 ?? row.有效触达数) || 0;
...
function teacherOutputRows(rows: DataRow[]) {
  return rows.map((row) => ({
    ...
    助理主管: row.助理主管 || "",
    应发送条数: row.应发送数 || 0,
    已发送数: row.已发送数 ?? row.有效触达数 ?? 0,
    发送差额: row.发送差额 ?? row.触达差额 ?? 0,
    申诉数: row.申诉数 || 0,
    发送率: row.发送率 || row.触达完成率 || "0.0%",
    是否达标: row.是否达标 || "否",
  }));
}
...
function explanationRows(
  ...
  const rows: DataRow[] = [
    ...
    { 项目: "发送率口径", 值: "教师及以上维度以聊天质检汇总文件为准，按“员工触达客户数+员工触达群数”累计已发送数；已发送数按该老师应发送数封顶，发送率=已发送数/应发送数" },
    ...
    { 项目: "申诉公示规则", 值: "上传申诉文件后，按教师姓名+学生姓名匹配分母；先判定聊天是否已发送，已发送优先展示为“是”并计入学员明细参考；未发送但申诉原因或状态选择“已发送”的行按“是”计入应发送；其他申诉行标注“已申诉”、展示申诉原因，且不计入应发送和发送率分母；各汇总页单独公示申诉数" },
  ];
...
export function buildReminderOutput(
  ...
  const sheets: SheetDefinition[] = [
    ...
    },
    {
      ...
      widths: { 教研组: 14, 助理主管: 18, 应发送数: 10, 已发送数: 12, 发送率: 13, "是否达标（80%）": 15 },
